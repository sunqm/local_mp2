...
#bases   = ['cc-pvdz','cc-pvtz','cc-pvqz','cc-pv5z','aug-cc-pvdz','aug-cc-pvtz','aug-cc-pvqz'] #,'aug-cc-pv5z']
...
def orbital_partitioning(mol,fragments,shells,verbose):
    ...
    for s in species:
        ...
        print ">>> T_atom"
        for ii in range(T_atom[0].shape[1]):
          for jj in range(T_atom[0].shape[0]):
            print jj,ii,T_atom[0][jj,ii]
...
def DMET_wrap(atoms,basis,charge,spin,fragments,fragment_spins,shells,nfreeze,method,thresh,parallel): 
    ...
    print ">>> CORE ORIBTALS"
    for i in range(Cf_core.shape[1]):
     for j in range(Cf_core.shape[0]):
      print j,i,Cf_core[j,i]

    print ">>> VALENCE ORIBTALS"
    for i in range(Cf_x.shape[1]):
